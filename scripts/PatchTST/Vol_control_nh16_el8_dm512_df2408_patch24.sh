...
do
    ...
    for target_vol in 90 180 270 360    # GIST, Miryang, Germany
    do
        ...
        if [ "$target_vol" = 14 ]; then
            root_path_name="/ailab_mat/dataset/PV/${data_name}/processed_data_${data_type}_2weeks/"
        elif [ "$target_vol" = 30 ]; then
            root_path_name="/ailab_mat/dataset/PV/${data_name}/processed_data_${data_type}_1month/"
        else
            root_path_name="/ailab_mat/dataset/PV/${data_name}/processed_data_${data_type}_$(($target_vol / 30))months/"
        fi

        setting_name="DKASC($source_vol)_${data_name}(${target_vol})_${data_type}_${model_name}_sl${seq_len}_pl${pred_len}_ll${label_len}_nh${n_heads}_el${e_layers}_dm${d_model}_df${d_ff}_patch${patch_len}"
