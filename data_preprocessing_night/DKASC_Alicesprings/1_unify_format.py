...
def combine_wind_speed_data(raw_weather_list, save_path):
    ...
    for weather_file in raw_weather_list:
        ...
        wind_speed_dfs.append(df_weather)
        
    # 데이터프레임들을 하나로 합칩니다.
    combined_weather_df = pd.concat(wind_speed_dfs, ignore_index=True)
    combined_weather_df.sort_values('timestamp', inplace=True)
    combined_weather_df = combined_weather_df[['timestamp', 'Wind_Speed']]
    combined_weather_df['Wind_Speed'] = pd.to_numeric(combined_weather_df['Wind_Speed'], errors='coerce')
    combined_weather_df['Wind_Speed'] = combined_weather_df['Wind_Speed'] * (1000 / 3600) # km/h to m/s
...
def combine_into_each_site(file_path, index_of_site,
                           save_dir, combined_weather_hourly):
...
if __name__ == '__main__':
    ...
    save_dir = os.path.join(project_root, 'data/DKASC_AliceSprings/uniform_format_data')
    ...
    for i, file_path in tqdm(enumerate(raw_file_list), desc=f'Processing'):
